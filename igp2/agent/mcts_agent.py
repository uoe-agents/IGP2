...
class MCTSAgent(MacroAgent):
    ...
    def update_observations(self, observation: Observation):
        ...
        for aid, agent_state in frame.items():
            try:
                # TODO check that this will handle disapearing and reaparing agents successfully

                # TODO we should probably remove an agent from the buffer if he hasn't been in view for a while
                # could do this by comparing the observation time with the last time each agent was observed and
                # removing stale agents
                self._observations[aid][0].add_state(agent_state)
            except KeyError:
                self._observations[aid] = (StateTrajectory(fps=self._fps), frame)

        for aid in self._observations.keys():
